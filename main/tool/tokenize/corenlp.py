...
class StanfordCoreNLP:
    ...
    def ner(self, sentence):
        ...
        for s in r_dict['sentences']:
            for token in s['tokens']:
                ...
                character_offset.append((token['characterOffsetBegin'], token['characterOffsetEnd']))
        return words, pos_tags, ner_tags, character_offset
    ...
    def dependency_parse(self, sentence):
        r_dict = self._request('depparse,ner', sentence)
        arcs = [(dep['dep'], dep['governor'], dep['dependent']) for s in r_dict['sentences'] for dep in
                s['basicDependencies']]
        words = [token['word'] for s in r_dict['sentences'] for token in s['tokens']]
        postags = [token['pos'] for s in r_dict['sentences'] for token in s['tokens']]
        word_character_offset = [(token['characterOffsetBegin'], token['characterOffsetEnd']) for s in
                                 r_dict['sentences'] for token in s['tokens']]
        netags = [token['ner'] for s in r_dict['sentences'] for token in s['tokens']]
        return arcs, words, postags, netags, word_character_offset
...
#                s['basicDependencies']]
    ...
    def openie(self, sentence):
        ...
        openie_result = []
        for child_dict in r_dict['sentences']:
            if 'openie' in child_dict:
                openie_result.extend(child_dict['openie'])
        return openie_result
        
    def coref(self, sentence):
        r_dict = self._request('coref', sentence)
        return r_dict
